...
Cyfe is a handy SaaS utility to build metrics and KPI dashboards out of "widgets." Hubrix has no special relationship with Cyfe, we're just a happy user. https://www.cyfe.com/
...
* The script depends on [sync-request](https://www.npmjs.com/package/sync-request) which is not recommanded for production use. Then again, this is a `cron` script with a typically very short execution lifespan, not a complex service managing a bunch of asynchronous threads.
